...
from dqn.dqn_collections import DeltaTransition, Transition, FingerprintDeltaTransition


def optimise(policy_dqn, target_dqn, replay_memory, optimiser, optimiser_device, gamma_tensor, batch_size, priority_coefficient, weighting_coefficient):
    if len(replay_memory.memory) < batch_size:
        return

    mem_size = len(replay_memory.memory)

    if "priority" in replay_memory.memory_type:
        ...
        batch = DeltaTransition(*zip(*transitions)) if replay_memory.memory_type == "priority" else FingerprintDeltaTransition(*zip(*transitions))

        # for calculating reward normalisation:

        r_tr = DeltaTransition(*zip(*replay_memory.sample(int(len(replay_memory) / 4)))).reward if replay_memory.memory_type == "priority" else FingerprintDeltaTransition(*zip(*replay_memory.sample(int(len(replay_memory) / 4)))).reward
        r_mean = np.mean(r_tr)
        r_std = np.std(r_tr)
