...
np.savetxt(os.getcwd() + "/states.txt", states)
...
for r1_pos in range(0, num_segments):
    for r2_pos in range(0, num_segments):
        for r3_pos in range(0,num_segments):
                        state = np.hstack(([r1_pos, r2_pos, r3_pos, 0, 0, 0], damaged_segments))
                        initial_state_indices.append(states_dict[str(state)])


print(f"Number of initial states: {len(initial_state_indices)}")
