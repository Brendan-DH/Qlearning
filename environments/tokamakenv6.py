...
class TokamakEnv6(gym.Env):
    ...
    def _get_obs(self):
        ...
        obs["elapsed_ticks"] = self.elapsed_ticks

        return obs

    def _get_info(self):
        info = {}
        info["elapsed_ticks"] = self.elapsed_ticks
    ...
    def step(self, action):
        ...
        if(blocked_actions[action]):
            ...
            reward = 0.0
            current_action = "wait"
        # elif(self.elapsed_ticks == self._max_episode_steps):
        #     print("timeout")
        #     reward = -1000
        else:
            ...
            if (rel_action == 2):  # engage robot, complete task
                for i in range(len(self._goal_locations)):  # iterate over locations and mark appropriate goals as done
                    if(self._goal_locations[i] == current_location and self._goal_instantiations[i] == 1):
                        ...
                        reward += 1000 - ((self.elapsed_ticks * self.num_robots) / self._max_episode_steps) * 500
    ...
    def _render_frame(self):
        ...
        for i in range(self.num_goals):
            ...
            if(self._goal_checked[i] == 0):
                ...
                colour = (255,0,0)
            elif(self._goal_checked[i] == 1):
                if(self._goal_resolutions[i] == 0):
                    text = font.render("0", True, (255,255,255))
                    colour = (200,200,200)
                elif(self._goal_instantiations[i] == 1):
                    text = font.render("1", True, (255,255,255))
                    colour = (0,200,0)
                elif(self._goal_instantiations[i] == 0):
        ...
        print(f"""
              epoch: {self.elapsed_ticks}-{np.sum(self._robot_clocks)}
        ...
        canvas.blit(font.render("t=" + str(self.elapsed_ticks), True, (0,0,0)), rect)
        # most recent actions
