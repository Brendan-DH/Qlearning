...
def template_engage(env, state_dict, robot_no):
    """
    ...
    state_dict = state_dict.copy()
    if state_dict[f"robot{robot_no} engagement"] < 9:
        state_dict[f"robot{robot_no} engagement"] += 1
    ...
    for i in range(env.unwrapped.num_goals):
        ...
        if ((state_dict[f"goal{i} location"] == state_dict[f"robot{robot_no} location"]) and state_dict[f"goal{i} active"] == 1):
            ...
            prob1 = (state_dict[f"goal{i} completion probability"] * 0.25) * state_dict[f"robot{robot_no} engagement"]
...
def t_model(env, state_dict, robot_no, action_no):
    ...
    if (action_no == 0 or action_no == 1):
        ...
        p, s = template_engage(env, state_dict, robot_no)
    elif (action_no == 3):
    ...
    for state in s:
        ...
        if (action_no != 3):
            state[f"robot{robot_no} engage"] = 0
