...
def template_complete(env, state_dict, robot_no):
    ...
    for i in range(env.unwrapped.num_goals):
        ...
        if (state_dict[f"goal{i} location"] == state_dict[f"robot{robot_no} location"]):
            ...
            state1[f"robot{robot_no} clock"] = state1[f"robot{robot_no} clock"] + 1

            # failure to complete goal
            prob2 = 1 - prob1
            state2 = state_dict.copy()
            state2[f"goal{i} active"] = 1
            state2[f"robot{robot_no} clock"] = state2[f"robot{robot_no} clock"] + 1
...
def r_model(env, old_state_dict, action_no, next_state_dict):
    ...
    if (b_model(env, old_state_dict, old_state_dict["clock"])[action_no] == 1):
        return 0
    ...
    # if (next_state_dict[(env.unwrapped.num_robots * 2) + 1] > robot_stamina):
    #     reward -= 50 * (robot_stamina - (next_state_dict[(env.unwrapped.num_robots * 2) + 1]))

    # rewards for completing goals
    for i in range(env.unwrapped.num_goals):
        # check if goal went from active to inactive
        if (old_state_dict[f"goal{i} active"] == 1 and next_state_dict[f"goal{i} active"] == 0):
