...
def template_complete(env, state_dict, robot_no):
    ...
    for i in range(env.unwrapped.num_goals):
        ...
        if (state_dict[f"goal{i} location"] == state_dict[f"robot{robot_no} location"]):
            ...
            # state1[f"robot{robot_no} fatigue"] = state1[f"robot{robot_no} fatigue"] + 1

            # failure to complete goal
            prob2 = 1 - prob1
            state2 = state_dict.copy()
            state2[f"goal{i} active"] = 1
            # state2[f"robot{robot_no} fatigue"] = state2[f"robot{robot_no} fatigue"] + 1
...
def b_model(env, state_dict, robot_no):
    ...
    # for robot_index in range(env.unwrapped.num_robots):
    #     if robot_no == robot_index:
    #         continue
    #     elif abs(min(active_robot_loc - state_dict[f"robot{robot_index} location"], env.unwrapped.size - (active_robot_loc - state_dict[f"robot{robot_index} location"]))) == 1:
    #         other_robot_adjacent = True
    #         break
    #
    # if not other_robot_adjacent:
    for goal_index in range(env.unwrapped.num_goals):
        if (state_dict[f"goal{goal_index} location"] == active_robot_loc and state_dict[f"goal{goal_index} active"] == 1):
